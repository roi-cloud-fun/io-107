...
resource "null_resource" "lab1_seed" {
  ...
  provisioner "local-exec" {
    command = <<-EOT
      ...
      cd "$WORK/cc"
      git init -q -b main
      git config user.email "labforge@example.invalid"
      git config user.name  "labforge-seed"
      git add -A
...
resource "kubernetes_namespace" "lab1" {
  ...
  metadata {
    ...
    labels = {
      "labforge.io/course"  = "io107"
      "labforge.io/lab"     = "lab1"
      "labforge.io/student" = var.student_id
    }
  }
